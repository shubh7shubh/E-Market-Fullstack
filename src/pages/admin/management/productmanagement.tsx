...
const Productmanagement = () => {
  ...
  if (params) console.log(params.id, "fsdhfsdqjk")
  ...
  const { price, photo, name, stock, category } = data?.singleProduct || {
    photo: "",
    category: "",
    name: "",
    stock: 0,
  ...
  // const { category, name, photo, price, stock } = product
  ...
  const submitHandler = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const res = await updateProduct({ formData, userId: user?._id!, productId: data?.singleProduct._id! })
  ...
  const deleteHandler = async () => {
    ...
    const res = await deleteProduct({ userId: user?._id!, productId: data?.singleProduct._id! })
